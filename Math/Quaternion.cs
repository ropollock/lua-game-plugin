...
namespace Lou {
    ...
    /// Does implicit conversions to and from UnityEngine.Quaternion
    ///
    /// Uses code from:
    ...
    public struct Quaternion : IEquatable<Quaternion> {
        ...
        public Vector3 eulerAngles {
            get { return Quaternion.ToEulerRad(this) * radToDeg; }
            set { this = Quaternion.FromEulerRad(value * degToRad); }
        }
        ...
        public static Vector3 AngularVelocity(Quaternion previousRotation, Quaternion currentRotation,
            float deltaTime) {
            Quaternion deltaRotation = currentRotation * Inverse(previousRotation);
        ...
        public string ToString(string format) {
            return string.Format("({0}, {1}, {2}, {3})", this.x.ToString(format), this.y.ToString(format),
                this.z.ToString(format), this.w.ToString(format));
        }
